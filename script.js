...
// ];
  ...
  if (selectedWord.includes(letter)) {
    ...
    updateWordDisplay(); // Alterado para usar a nova função
  } else {
      playSound("wrong");
      wrongGuesses++;
      wrongDisplay.textContent = wrongGuesses;
...
function checkWordLength() {
  ...
  // Garante que o conteúdo seja atualizado
  wordDisplay.textContent = displayedWord.join(" ");
...
// Função para verificar e ajustar o layout
function checkLayout() {
  const isMobile = window.innerWidth <= 768;
  const wordContainer = document.querySelector('.word-container');
  
  if (isMobile) {
    wordContainer.classList.add('mobile-view');
  } else {
    wordContainer.classList.remove('mobile-view');
  }
...
// Inicializa o layout quando o jogo começa
initGame();
checkLayout();